...
pro fel_randomimage_displayImage, filename, pState
		...
		if (*pstate).use_pixmap then begin
			data = pixelRemap(data, (*pstate).pixmap_x, (*pstate).pixmap_y, (*pstate).pixmap_dx)
		endif
	...
	case ev.id of 
		...
		sState.button_auto : begin
			if (*pstate).autoNext eq 1 then begin
	      		...
	      		widget_control, sState.button_auto, set_value='Stop'
	      		(*pstate).autoNext = 1
				widget_control, sState.menu_random,  timer=0.1
			endelse
